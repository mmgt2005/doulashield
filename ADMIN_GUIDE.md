...
**Assigning a service to the billing agency:** Click **Assign to Agency** on the right side of any service row to hand that service off to the agency's billing admin. The provider must already be linked to a billing agency — assign the provider first if needed. Once assigned, the button becomes a filled **✓ Assigned to Agency** badge and the service gains an "Assigned by DoulaShield" label in the billing admin's My Providers panel. Assigning automatically enables the Enrollment Tier on the billing provider if it is not already active, so the billing admin can manage the service immediately without a separate admin step. Click the button again to unassign (the Enrollment Tier remains enabled). Use this when you have started a service to get enrollment underway and want the billing admin to take over day-to-day task tracking.
...
- **Mark a stage complete** *(Enrollment Tier required)* — once every task in a service is checked complete, a "Mark [Stage] Complete →" prompt appears inside the expanded service panel. Clicking it opens a modal to record the required completion data: cert date for PCB, 10-digit NPI for NPPES, PROMISe™ enrollment date (plus optional PROMISe™ ID and CAQH ID) for Stage 2, or contracted date for MCO Contracting. On save, the provider's credential record is updated and the next stage is unlocked. A green completion banner then appears with a one-click "Start [Next Stage] →" shortcut so billing admins can move through the full pipeline (PCB → NPPES → Stage 2 → MCO Contracting) without leaving My Providers.
